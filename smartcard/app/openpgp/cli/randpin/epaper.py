...
class _Login1ManagerMessageGenerator(jeepney.wrappers.MessageGenerator):
    """
    DBus message generator for systemd logind.

    Generated by jeepney version 0.7.1 and then trimmed down to the necessary
    methods.
    """
    ...
    def Inhibit(self, what, who, why, mode):
        return jeepney.wrappers.new_method_call(
            self,
            'Inhibit',
            'ssss',
            (what, who, why, mode),
...
class _Login1ManagerIdleInhibitor:
    """
    Minimal synchronous systemd logind dbus connector which allows inhibiting
    idle.
    """
    __connection = None
    __inhibitor = None
    ...
    def __init__(self):
        self.__message_generator = _Login1ManagerMessageGenerator()

    def open(self):
        """
        Opens DBus connection.
        """
        if self.__connection is None:
            self.__connection = jeepney.io.blocking.open_dbus_connection(
                bus='SYSTEM',
                enable_fds=True,
            )
    ...
    def close(self):
        """
        Calls self.allow() and closes the DBus connection.
        """
    ...
    def inhibit(self):
        """
        Inhibits idle, if not already inhibited.
        """
        if self.__inhibitor is None:
            self.__inhibitor = self.__connection.send_and_get_reply(
                self.__message_generator.Inhibit(
                    what='idle',
                    # XXX: is there a better name ?
                    # Ex: current process' unit name.
                    who='python-smartcard',
                    why='Connected to USB host',
                    mode='block',
                ),
            ).body[0]
    ...
    def allow(self):
        """
        Re-allows idle, if not already allowed.
        """
        inhibitor = self.__inhibitor
        if inhibitor is not None:
            inhibitor.close()
            self.__inhibitor = None
...
class ICCDFunctionWithRandomPinDisplay(ICCDFunction):
    ...
    def __init__(
        ...
        if has_battery:
            ...
            self.__idle_inhibitor = _Login1ManagerIdleInhibitor()
    ...
    def displayBattery(self):
        ...
        if state == self.__battery_last_state:
            return False
        if charging != self.__battery_last_state[1]:
            # Note: using charging status as a surogate for connection to USB
            # host: UDCs typically detect disconnect when vbus goes low, but on
            # my reference board (raspberry pi zero) vbus is bridged to system
            # power bus, so it will essentially never go low (from what the CPU
            # can see anyway) - unless there is some non-standard way to power
            # the pi from the 3.3v bus only.
            # This means that plugging the pi on a charging-only port will
            # prevent it from shutting down, despite being an unnecessary power
            # drain.
            if charging:
                self.__idle_inhibitor.inhibit()
            else:
                self.__idle_inhibitor.allow()
